...
#ifndef SDLT_AOS_1D_CONTAINER_H
#define SDLT_AOS_1D_CONTAINER_H
...
#include "internal/aos1d_by_stride/const_accessor.h"
#include "internal/aos1d_by_stride/const_element.h"
#include "internal/aos1d_by_stride/element.h"
#include "internal/aos1d_by_struct/accessor.h"
#include "internal/aos1d_by_struct/const_accessor.h"
#include "internal/aos1d_by_struct/const_element.h"
#include "internal/aos1d_by_struct/element.h"
...
public:
    ...
    static const int align_d1_on_index = 0;
    typedef AllocatorT allocator_type;
    typedef size_t size_type;

    template <typename OffsetT = no_offset> using accessor = typename std::conditional<AccessByT == access_by_stride,
        internal::aos1d_by_stride::accessor<PrimitiveT, OffsetT>,
        internal::aos1d_by_struct::accessor<PrimitiveT, OffsetT>
        >::type;

    template <typename OffsetT = no_offset> using const_accessor = typename std::conditional<AccessByT == access_by_stride,
        internal::aos1d_by_stride::const_accessor<PrimitiveT, OffsetT>,
        internal::aos1d_by_struct::const_accessor<PrimitiveT, OffsetT>
        >::type;

    // Support move constructor and move assignment
    // Allows swap to function efficiently
    SDLT_INLINE aos1d_container(aos1d_container&& donor);
    SDLT_INLINE aos1d_container & operator=(aos1d_container&& donor);
    ...
    aos1d_container(const aos1d_container &) = delete;
    aos1d_container & operator = (const aos1d_container &) = delete;
    SDLT_NOINLINE aos1d_container clone() const;

    SDLT_INLINE aos1d_container(
    ...
    SDLT_INLINE const_accessor<int>
    ...
    SDLT_INLINE auto operator [](const int an_index_d1) const
        -> decltype(std::declval<aos1d_container>().const_access()[linear_index(an_index_d1)]);
    SDLT_INLINE auto operator [](const int an_index_d1)
        -> decltype(std::declval<aos1d_container>().access()[linear_index(an_index_d1)]);
    ...
    SDLT_INLINE auto operator [](const IndexD1T an_index_d1) const
        -> decltype(std::declval<aos1d_container>().const_access()[an_index_d1]);

    template<typename IndexD1T, typename = internal::enable_if_type<internal::is_linear_compatible_index<IndexD1T>::value>>
    SDLT_INLINE auto operator [](const IndexD1T an_index_d1)
        -> decltype(std::declval<aos1d_container>().access()[an_index_d1]);
    ...
    SDLT_INLINE void swap(aos1d_container &other);

    // std::vector relational operator compatibility
    SDLT_NOINLINE bool operator == (const aos1d_container &other) const;
    SDLT_NOINLINE bool operator != (const aos1d_container &other) const;
...
protected:  // member functions
    ...
    typedef internal::linear::element_mover<aos1d_container> element_mover;
...
template<typename PrimitiveT, access_by AccessByT, class AllocatorT>
aos1d_container<PrimitiveT, AccessByT, AllocatorT>::
aos1d_container(
    const int size_d1,
    buffer_offset_in_cachelines buffer_offset,
    const AllocatorT * an_allocator)
...
template<typename PrimitiveT, access_by AccessByT, class AllocatorT>
aos1d_container<PrimitiveT, AccessByT, AllocatorT>::
aos1d_container(aos1d_container&& donor)
//: organization(std::move(donor))
: m_size_d1(donor.m_size_d1)
...
aos1d_container<PrimitiveT, AccessByT, AllocatorT> &
aos1d_container<PrimitiveT, AccessByT, AllocatorT>::
operator=(aos1d_container&& donor)
{
    if (m_buffer != nullptr)
    {
...
aos1d_container(
    ...
    buffer_offset_in_cachelines buffer_offset,
    const AllocatorT & an_allocator)
: aos1d_container(size_d1, buffer_offset, &an_allocator)
{
...
aos1d_container(
    ...
    buffer_offset_in_cachelines buffer_offset,
    const AllocatorT & an_allocator)
: aos1d_container(other.size(), buffer_offset, &an_allocator)
{
    __SDLT_ASSERT(other.size() <= max_size());
    aos1d_const_accessor<PrimitiveT, AccessByT> source(other);
...
aos1d_container(
    ...
    buffer_offset_in_cachelines buffer_offset,
    const AllocatorT & an_allocator)
: aos1d_container(number_of_elements, buffer_offset, &an_allocator)
{
    __SDLT_ASSERT(number_of_elements < max_size());
    aos1d_const_accessor<PrimitiveT, AccessByT> source(other_array, number_of_elements);
    element_mover::copy_all_values_from(source, access());
}
...
template< typename IteratorT >
aos1d_container<PrimitiveT, AccessByT, AllocatorT>::
aos1d_container(IteratorT a_begin, const IteratorT an_end,
    buffer_offset_in_cachelines buffer_offset,
    const AllocatorT & an_allocator)
: aos1d_container(std::distance(a_begin, an_end), buffer_offset, &an_allocator)
{
...
clone() const
{
    aos1d_container dest_container(this->get_size_d1(), m_buffer_offset, m_allocator);
...
auto
aos1d_container<PrimitiveT, AccessByT, AllocatorT>::
const_access(fixed<OffsetT> offset) const
...
-> decltype(std::declval<aos1d_container>().const_access()[linear_index(an_index_d1)])
{
    ...
    //unsigned char const * charData = this->m_data;
    //std::cout << "redData() " << int(charData[0]) << "," << int(charData[1]) << "," << int(charData[2]) << std::endl;

    linear_index xindex(an_index_d1);

    return typename const_accessor<>::template const_element< decltype(xindex) >(m_data, xindex, internal::disambiguator());
}
...
-> decltype(std::declval<aos1d_container>().access()[linear_index(an_index_d1)])
{
    ...
    return typename accessor<>::template element< decltype(xindex) >(m_data, xindex, internal::disambiguator());
}
...
-> decltype(std::declval<aos1d_container>().const_access()[an_index_d1])
{
    ...
    return const_accessor<>::template const_element< IndexD1T >(m_data, an_index_d1);
}
...
-> decltype(std::declval<aos1d_container>().access()[an_index_d1])
{
    ...
    return typename accessor<>::template element< IndexD1T >(m_data, an_index_d1, internal::disambiguator());
}
...
{
    #if SDLT_DEBUG
        size_t required_size = sizeof(PrimitiveT)*a_digest.size_d1;
        __SDLT_ASSERT(a_digest.array_size >= required_size);
    #endif

    unsigned char *data = (an_offload_array + a_digest.data_offset_in_array);

    return accessor<>(data, a_digest.size_d1, no_offset(), internal::disambiguator());
...
do_reserve(internal::capacity new_capacity)
{
    ...
    // with it.
    aos1d_container dest_container(new_capacity, this->get_size_d1(), m_buffer_offset, m_allocator);
    auto source = const_access();
    element_mover::copy_all_values_from(source, dest_container.access());
...
template<typename PrimitiveT, access_by AccessByT, class AllocatorT>
template <typename... ArgsT>
typename aos1d_container<PrimitiveT, AccessByT, AllocatorT>::iterator
aos1d_container<PrimitiveT, AccessByT, AllocatorT>::
emplace (const_iterator iterator_pos, ArgsT&&... iArgs)
{
    ...
    // First we need to shift all the elements up
    element_mover::shift_elements_forwards(access(), iterator_pos.m_index, new_element_count);

    //access()[an_index] = a_value;
    // Placement new, possibly using move constructor
    new (&reinterpret_cast<PrimitiveT *>(m_data)[iterator_pos.m_index]) PrimitiveT(std::forward<ArgsT>(iArgs)...);
...
emplace_back(ArgsT&&... iArgs)
{
    ...
    // Placement new, possibly using move constructor
    new (&reinterpret_cast<PrimitiveT *>(m_data)[index_to_assign]) PrimitiveT(std::forward<ArgsT>(iArgs)...);
}
...
bool
aos1d_container<PrimitiveT, AccessByT, AllocatorT>::
operator == (const aos1d_container &other) const
{
    SDLT_INLINE_BLOCK
    {
        if (size() != other.size())
...
do_allocation(internal::capacity new_capacity)
{
    if (new_capacity.count() != 0)
    {
        ...
        size_t required_size_in_bytes = sizeof(PrimitiveT)*new_capacity.count();
...
using __SDLT_IMPL::aos1d_container;
} // namespace sdlt

#endif // SDLT_AOS_1D_CONTAINER_H
