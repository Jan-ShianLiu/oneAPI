...
#ifndef SDLT_AOS_1D_BY_STRIDE_CONST_ACCESOR_H
#define SDLT_AOS_1D_BY_STRIDE_CONST_ACCESOR_H
...
public:
    ...
    template<typename IndexD1T> using const_element = aos1d_by_stride::const_element< PrimitiveT, IndexD1T >;
    ...
    SDLT_INLINE
    const_accessor(const accessor<PrimitiveT, OffsetT> & iAccessor)
    : m_data(iAccessor.m_data)
    , m_size_d1(iAccessor.m_size_d1)
    , m_offset(iAccessor.m_offset)
    {
    }
    ...
    SDLT_INLINE
    const_accessor(const std::vector<PrimitiveT, StlAllocatorT> &other,
            OffsetT offset = default_offset<OffsetT>::value)
    : m_data(reinterpret_cast<const unsigned char *>(&other[0]))
    ...
    const_accessor(
        ...
        const int size_d1,
        OffsetT offset = default_offset<OffsetT>::value)
    : m_data(reinterpret_cast<unsigned char const *>(a_data))
    , m_size_d1(size_d1)
    , m_offset(offset)
    {
    }
...
#endif // SDLT_AOS_1D_BY_STRIDE_CONST_ACCESOR_H
