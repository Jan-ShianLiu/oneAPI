...
"MSWin32-x64-multi-thread-10.0.14393" or
	die "Errno architecture (MSWin32-x64-multi-thread-10.0.14393) does not match executable architecture ($Config{'archname'}-$Config{'osvers'})";
...
our %EXPORT_TAGS = (
    ...
    WINSOCK => [qw(
	WSAEDESTADDRREQ WSAEPROVIDERFAILEDINIT WSAENOTEMPTY WSAEFAULT
	WSAEUSERS WSAEWOULDBLOCK WSAEINVAL WSAENOTSOCK WSAEPROCLIM
	WSAEHOSTDOWN WSAEREFUSED WSAEAFNOSUPPORT WSAENOBUFS
	WSAEINVALIDPROVIDER WSAEPFNOSUPPORT WSAELOOP WSAEINVALIDPROCTABLE
	WSAESOCKTNOSUPPORT WSAENETUNREACH WSAESHUTDOWN WSAECANCELLED
	WSAENOMORE WSAEADDRNOTAVAIL WSAETIMEDOUT WSAEMFILE WSAECONNREFUSED
	WSAECONNABORTED WSAEMSGSIZE WSAENETRESET WSAEACCES WSAEREMOTE
	WSAENOTCONN WSAECONNRESET WSAEALREADY WSAESTALE WSAEHOSTUNREACH
	WSAEPROTONOSUPPORT WSAEADDRINUSE WSAETOOMANYREFS WSAEISCONN
	WSAEPROTOTYPE WSAEINTR WSAEDQUOT WSAEBADF WSAEDISCON WSAENAMETOOLONG
	WSAENETDOWN WSAEOPNOTSUPP WSAEINPROGRESS WSAENOPROTOOPT
    )],
);
