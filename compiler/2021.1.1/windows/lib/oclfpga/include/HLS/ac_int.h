...
#error Intel High Level Synthesis compiler is required to include this header file. To compile with other compilers, include "ref/ac_int.h" instead.
#endif
...
inline std::string to_string(ap_int<N> value, int base, bool sign_mag = false) {
  ...
  if (value >= 0) {
    // If the user declares a positive number but sign_mag is off,
    // the way to represent the number as positive is to have its MSB to be 0
    // instead of 1.
    if (!sign_mag) {
      ap_uint<N> t = v;
      if (base == 10 || base == 8) {
    ...
    // If sign_mag is false, negative number gets rid of neg sign, but it goes
    // through 1's complement + 1
    if (!sign_mag && base != 10) {
      ap_uint<N> t = -v;
      t = ~t + 1;
      return prefix + to_string(t, base);
...
public:
  ...
  constexpr void mod(const iv<N2, S2> &op2, iv<Nr, Sr> &r) const {
    r.value = value % op2.value;
  }
  constexpr void increment() { value += 1; }
  constexpr void decrement() { value -= 1; }
