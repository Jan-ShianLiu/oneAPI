...
template <int E, int M> struct HLS_FP_Supported {
  static const int value = isPrecisionSupported< E, M,
...
class hls_float {
  ...
  static_assert( Rnd==fp_config::FP_Round::RNE || Rnd==fp_config::FP_Round::RZERO,
                "The compiler currently only supports round to nearest(even) and to zero");
...
public:
  ...
  constexpr ac_int<E + M + 1, false> get_bits() const {
    return fp_bit_value;
  }
  ...
  template <int E2, int M2, RD_t ret_RD=RD_t::RNE> struct return_type {
    enum {
      ...
      op_m = E == E2 ? VPF_MAX(M, M2) : E > E2 ? M : M2,
    };
  ...
  template <int E2, int M2, RD_t R2> hls_float<E, M, Rnd>
  &operator=(const hls_float<E2, M2, R2> &rhs) {
    internal::hls_vpfp_cast(rhs, *this);
    return *this;
  }
  ...
  template <int E2, int M2, RD_t R2, typename = std::enable_if_t<E2!=E || M2!=M>>
  ...
  operator+(const hls_float<E2, M2, R2> &rhs) const {
    ...
    if (unify) {
      arg1_t arg1(*this);
      arg2_t arg2(rhs);
      internal::hls_vpfp_add<DefaultAccuracy, DefaultSubnormal>(ret, arg1, arg2);
    } else {
      internal::hls_vpfp_add<DefaultAccuracy, DefaultSubnormal>(ret, *this, rhs);
    }
    return ret;
  }
  ...
  operator-(const hls_float<E2, M2, R2> &rhs) const {
    ...
    if (unify) {
      arg1_t arg1(*this);
      arg2_t arg2(rhs);
      internal::hls_vpfp_sub<DefaultAccuracy, DefaultSubnormal>(ret, arg1, arg2);
    } else {
      internal::hls_vpfp_sub<DefaultAccuracy, DefaultSubnormal>(ret, *this, rhs);
    }
    return ret;
  }
  ...
  operator*(const hls_float<E2, M2, R2> &rhs) const {
    ...
    if (unify) {
      arg1_t arg1(*this);
      arg2_t arg2(rhs);
      internal::hls_vpfp_mul<DefaultAccuracy, DefaultSubnormal>(ret, arg1, arg2);
    } else {
      internal::hls_vpfp_mul<DefaultAccuracy, DefaultSubnormal>(ret, *this, rhs);
    }
    return ret;
  }
  ...
  operator/(const hls_float<E2, M2, R2> &rhs) const {
    ...
    if (unify) {
      arg1_t arg1(*this);
      arg2_t arg2(rhs);
      internal::hls_vpfp_div<DefaultAccuracy, DefaultSubnormal>(ret, arg1, arg2);
    } else {
      internal::hls_vpfp_div<DefaultAccuracy, DefaultSubnormal>(ret, *this, rhs);
    }
    return ret;
  }
  ...
  hls_float<E, M, Rnd> &operator+=(const hls_float<E2, M2, R2> &rhs) {
    ...
    if (unify)
      *this = this->operator+(rhs);
      ...
      internal::hls_vpfp_add<DefaultAccuracy, DefaultSubnormal>(*this, *this, rhs);
    }
    return *this;
  }
  ...
  hls_float<E, M, Rnd> &operator-=(const hls_float<E2, M2, R2> &rhs) {
    ...
    if (unify)
      *this = this->operator-(rhs);
      ...
      internal::hls_vpfp_sub<DefaultAccuracy, DefaultSubnormal>(*this, *this, rhs);
    }
    return *this;
  }
  ...
  hls_float<E, M, Rnd> &operator*=(const hls_float<E2, M2, R2> &rhs) {
    ...
    if (unify)
      *this = this->operator*(rhs);
      ...
      internal::hls_vpfp_mul<DefaultAccuracy, DefaultSubnormal>(*this, *this, rhs);
    }
    return *this;
  }
  ...
  hls_float<E, M, Rnd> &operator/=(const hls_float<E2, M2, R2> &rhs) {
    ...
    if (unify)
      *this = this->operator/(rhs);
      ...
      internal::hls_vpfp_div<DefaultAccuracy, DefaultSubnormal>(*this, *this, rhs);
    }
    return *this;
  }
  ...
  template <fp_config::FP_Accuracy Accuracy = DefaultAccuracy,
            fp_config::FP_Subnormal Subnormal = DefaultSubnormal,
            int E1, int M1, int E2, int M2, RD_t R1, RD_t R2>
  ...
  std::string get_str (int base=10, fp_config::FP_Round rnd=RD_t::RNE) const {
#ifdef HLS_X86
  ...
  // internal usage, not useful for application development
  template <int APWidth> using _fp_as_ap_uint = unsigned int __attribute__((__ap_int(APWidth)));
  const _fp_as_ap_uint<E + M + 1> _get_bits_ap_uint() const { return fp_bit_value._get_value_internal(); }
...
private:
  ...
  hls_float<E,M, Rnd> get_next() const {
    ...
    if (is_nan()){
      ...
      if (!get_sign_bit()){
        ...
        ac_int<M+1, false> new_m = get_mantissa() | (ac_int<M+1, false>(1) << M);
        ac_int<E, false> new_e = get_exponent();
  ...
  ac_int<hls_long_double_bit_width, false> constexpr
  LdoubleToHLSLdouble( long double x) {
    ...
    rep.template set_slc<wE + 1>(wM, Caster.casted.template slc<wE + 1>(wM + 1));
    return rep;
#endif
  }

  long double constexpr
  HLSLDoubleToLongDouble(const ac_int<hls_long_double_bit_width, false> x) {
...
#ifndef _HLS_EMBEDDED_PROFILE
template <int E, int M, fp_config::FP_Round Rnd>
std::ostream & operator << (std::ostream &out, const hls_float<E, M, Rnd> & number) {
#ifdef HLS_X86
  out << number.get_str();
#endif
  return out;
