...
// Currently, the math functions for hls_float only supports up to double precision
template <int E, int M>
static constexpr void validateFPSupport() {
  constexpr int MaxSupportedExponentWidth = 11;
  constexpr int MaxSupportedMantissaWidth = 52;
  static_assert( E <= MaxSupportedExponentWidth &&\
                 M <= MaxSupportedMantissaWidth,
                "Math function does not support this exponent or mantissa bit widths");
}

template <int E, int M>
static constexpr void validateEarlyFPSupport() {
...
template <int E1, int M1, int E2, int M2,
          ihc::fp_config::FP_Round Rnd1, ihc::fp_config::FP_Round Rnd2>
