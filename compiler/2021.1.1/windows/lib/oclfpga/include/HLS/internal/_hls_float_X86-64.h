...
void copy_array_to_ac_int(
  ...
  for (int idx = 0; (idx * 8) < N; idx++) {
    if (N - idx * 8 >= 8) {
      ...
      ac_val.set_slc(8 * idx, ac_int<last_size, false>(arr_val[idx] & bit_mask));
    }
  }
}
...
std::string get_str_x86(hls_float<E, M, Rnd> x, int base, int rounding) {
  if (base < 2 || base > 16) {
    std::cerr << "Unsupported base type" << std::endl;
    exit(-1);
  }
  ...
  // the internal get_str returns us the mantissa and where the decimal point should be
  // normally we need mantissa + 1 at least, but has to be greater than 7
...
hls_float<E1, M1> pow_vpfp_impl(hls_float<E1, M1, Rnd1> const &x,
  ...
  // for infinity, handle the corner case which we get infinity but should be -inf
  // this happens when the exponent (y) is odd negative integer, x is less than 0, and -0
  if (result.is_inf() && !result.get_sign_bit() && x.get_sign_bit() &&  y.get_sign_bit() ) {
    bool is_float_int_odd = false;
    ac_int<E2, false> exp = y.get_exponent();
    ac_int<M2, false> man = y.get_mantissa();
    ac_int<E2+1, true> exp_real = ac_int<E2+1, true>(exp) - \
