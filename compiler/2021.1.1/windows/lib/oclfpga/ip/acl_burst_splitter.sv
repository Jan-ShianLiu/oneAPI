...
module acl_burst_splitter #(
    ...
    //derived parameters
    parameter int unsigned DATA_WIDTH = 8*BYTEENABLE_WIDTH,
    parameter int unsigned ADDRESS_BITS_PER_WORD = $clog2(BYTEENABLE_WIDTH)    // how many lower bits of the byte address are stuck at 0 to ensure it is word aligned
) (
    input  wire                         clock,
    input  wire                         resetn,
    ...
    begin
        if (DATA_WIDTH != 8*BYTEENABLE_WIDTH)
            $fatal(1, "Illegal parameteriazation, DATA_WIDTH must be not be specified when instantiating this module, it must be left at its default value");
        
        if (ADDRESS_BITS_PER_WORD != $clog2(BYTEENABLE_WIDTH))
            $fatal(1, "Illegal parameteriazation, ADDRESS_BITS_PER_WORD must be not be specified when instantiating this module, it must be left at its default value");
    end
    ...
    `ACL_PARAMETER_ASSERT(BURST_BOUNDARY < ADDRESS_WIDTH)
    `ACL_PARAMETER_ASSERT(BURST_BOUNDARY == 0 || BURST_BOUNDARY >= ADDRESS_BITS_PER_WORD)
    `ACL_PARAMETER_ASSERT(USE_STALL_LATENCY == 0 || SPLIT_READ_BURSTS == 0)
    endgenerate
