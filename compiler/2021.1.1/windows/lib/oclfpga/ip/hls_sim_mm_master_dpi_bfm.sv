...
module hls_sim_mm_master_dpi_bfm(
  ...
  logic [SLAVE_WRITE_STREAM_DATA_WIDTH - 1 : 0] return_busy;
  task wait_for_not_busy;
    return_busy = 1;
    while (return_busy != 0) begin
      read_from_slave(64'b0);
  ...
  generate
    if (COMPONENT_CRA_SLAVE == 1) begin
      ...
      assign slave_busy = 1'b1;
    end
  endgenerate
  assign slave_busy_out = slave_busy;

  // for slave component, ready_for_write has to wait for slave_busy deasserts
  assign ready_for_write = ((!slave_busy) | !(COMPONENT_CRA_SLAVE == 1)) & ((slave_write_state == STATE_READY_FOR_WRITES) | wr_last_sanitized); //always ready out last write from slave stream
