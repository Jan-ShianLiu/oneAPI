...
/* If HYPER_PIPELINE==1, the AVMM slave to which this module connects must have a minimum waitrequest-allowance of at least (WAIT_REQUEST_INPUT_PIPE_DEPTH + 1 + ROOT_FIFO_STALL_IN_EARLINESS + NUM_AVM_OUTPUT_PIPE_STAGES). 
    ...
    If ENABLE_BSP_WAITREQUEST_ALLOWANCE=0 and HYPER_PIPELINE=1, the master-root-FIFO is instantiated to convert the BSP's non-waitrequest-allowance interface to the internal waiterquest-allownance interface used by the ring
*/
parameter NUM_AVM_OUTPUT_PIPE_STAGES = 1;  // Minimum value 1. Length of pipeline stages between root FIFOs and CCB. This can be increased for performance (note that the slave-side
  ...
  if (HYPER_PIPELINE == 0) begin : GEN_HYPER_PIPELINE_0
    ...
    */
    ...
    for (z=0;z<NUM_DIMM;z++) begin : GEN_MASTER_ROOT_FIFO
      ...
      // By default we do not use the master root FIFO if HYPER_PIPELINE=1. This requires support for waitrequest allowance on the AvalonMM interface.
      if (ENABLE_BSP_WAITREQUEST_ALLOWANCE) begin : GEN_DISABLE_MASTER_ROOT_FIFO
        ...
        assign master_root_fifo_almost_full[z] = (WAIT_REQUEST_INPUT_PIPE_DEPTH>0)? wait_request_input_pipe[WAIT_REQUEST_INPUT_PIPE_DEPTH][z] : i_avm_waitrequest[z];
        assign ecc_err_status_for_0[z] = 1'h0;
        assign ecc_err_status_for_1[z] = 1'h0;
      end else begin : GEN_ENABLE_MASTER_ROOT_FIFO

        hld_fifo #(
            .WIDTH                          (MASTER_ROOT_FIFO_WIDTH),
        ...
        assign {o_avm_byteenable[z], o_avm_address[z], o_avm_burstcount[z]} = master_root_fifo_data_out[z][WRITE_ROOT_FIFO_WIDTH+1]?
          master_root_fifo_data_out[z][MWIDTH_BYTES + O_AVM_ADDRESS_W+ BURST_CNT_W + MWIDTH - 1 : MWIDTH] :
          {{MWIDTH_BYTES{1'b1}},master_root_fifo_data_out[z][O_AVM_ADDRESS_W+BURST_CNT_W-1:BURST_CNT_W],master_root_fifo_data_out[z][BURST_CNT_W-1:0]};
    ...
    */
    ...
    if(NUM_ID == 1) begin : GEN_SINGLE_PORT    
      ...
      for(z=0; z<NUM_DIMM; z=z+1) begin : ROOT_FIFO_CONTROL_LOGIC
        ...
        */        
        assign write_root_fifo_rd_req_comb[z] = !master_root_fifo_almost_full[z] && !DISABLE_WR_RING;
        ...
        */
        ...
        if (ENABLE_LOW_LATENCY_WRITE_ACK) begin : GEN_INTERNAL_WRITE_ACK_LOW_LATENCY// Writes are prioritized over reads and we need to generate the writeack internally. Therefore we can generate the write-ack as the request leaves the ring. This is the earliest we can do it.
          ...
          */
          ...
          logic [BURST_CNT_W-1:0] write_ack_router_i_slave_avm_burstcount [NUM_DIMM];
          ...
          if (ENABLE_MULTIPLE_WR_RING_INT) begin : GEN_WRITE_ACK_PARALLEL_COMMANDS
            ...
            for (z=0; z<NUM_WRITE_ACK_COMMAND_INPUTS;z=z+1) begin : GEN_RANDOM_BLOCK_NAME_R192_2
              assign write_ack_avm_port_num[z]        = ROOT_PORT_MAP[current_mem_system_wr[z]][current_bank_within_mem_system_wr[z]]; // Target slave #
          ...
          for (z=0; z<NUM_DIMM; z=z+1) begin : GEN_RANDOM_BLOCK_NAME_R192_3
            assign write_ack_router_i_slave_avm_burstcount[z] = 1;
          end
          ...
          lsu_rd_back_n #(
            ...
            .i_slave_avm_burstcount (write_ack_router_i_slave_avm_burstcount),
            .i_slave_master_id      (wr_o_id),
            .i_slave_valid          (wr_dimm_en),
    ...
    logic [RD_ID_WIDTH-1:0]  lsu_rd_back_i_to_avm_id         [1];  // LSU ID, used when NUM_MASTER_COMMAND_INPUTS == 1. Otherwise the index simply indicates the master ID.
