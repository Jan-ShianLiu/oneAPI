...
sub get_platform($@) {
  ...
  } elsif ( $type eq "s10mx_ref" ) {
      return ( name     => "s10mx_ref",
        host     => "PCIe",
        pgm      => "Partial Reconfiguration",
        flow     => "QHD - Partial Reconfiguration",
        family   => "STRATIX 10",
...
sub migrate_platform_postcompile {
  ...
  if ( $platform{name} eq 's10_ref' and $platform{version} >= 18.0 ) {
    $platform{pr_base_id} = get_pr_base_id( "pr_base.id" );
    push(@fixes, "post_skipbak");
  }

  if ( $platform{name} eq 's10mx_ref' and $platform{version} >= 20.1 ) {
    $platform{pr_base_id} = get_pr_base_id( "pr_base.id" );
    push(@fixes, "post_skipbak");
  }
