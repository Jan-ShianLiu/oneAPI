...
void fftw_mpi_local_sizes(fftw_mpi_plan p,MKL_INT *n,MKL_INT *start,MKL_INT *n_out,MKL_INT *start_out,MKL_INT *size) {
	...
	*size=v;
	return;
error:
	fprintf(stderr,"CDFT error "PRINT_LI" in fftw_mpi_local_sizes wrapper\n",err);
	return;
}
